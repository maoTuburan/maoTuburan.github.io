							...
							<div class="col-md-12">
								<div class="form-floating mb-3">
									<select class="form-select" id="livestock_species" aria-label="livestock_species" name="livestock_species" required>
										@foreach(['Cattle', 'Swine Breeding','Carabao','Swine Fattening','Horse', 'Goat/Sheep'] as $currentLivestockSpecies)
											<option value="{{ $currentLivestockSpecies }}" {{ $livestock->livestock_species == $currentLivestockSpecies ? 'selected' : '' }}>{{ $currentLivestockSpecies }}</option>
										@endforeach
									</select>
