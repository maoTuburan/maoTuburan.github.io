$(function () {
	...
	let this_id;
	// modal
	let modal = $('#rice-modal');
	// start => datatable
	let table = $("#rice-table").DataTable({
		autoWidth: false,
		responsive: true,
		processing: true,
		serverSide: true,
		...
		ajax: {
			method: "GET",
			url: "/banner-programs/rices/table",
			dataType: "json",
	...
	// start => button delete
	$('body').on('click', '.delete-rice', function () {
		this_id = $(this).attr('data-id');
		modal.modal('show');
	});
	// end
	//start => modal button delete
	$('body').on('click', '#destroy-rice', function () {
		$.ajax({
			url: "/banner-programs/rices/" + this_id,
			type: "DELETE",
			dataType: "json",
			beforeSend: function () {
				buttons('destroy-rice', 'start');
			}
		})
